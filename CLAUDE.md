...
All on **staging**. Migrations `1750000000001`–`1750000000116` applied. **SOC migrations 104–110 are committed & pushed** (commit `20f2f8a`). MFS migrations 111–116 were added this session and **force-added to git but not yet committed** (commit only on explicit request). HBP 098–103 and PEDSURG 091–097 force-added earlier; VASC 085–090 committed.
...
4. **Proc_cpts for remaining departments** (OBGYN, OPHTHAL, ENT, TRS, UROL) not yet imported. (CTS, NS, GS, ORTHO, PRS, VASC, PEDSURG, HBP, SOC, **MFS** done.)
