...
### RESOLVED 2026-06-27 — UROL full ICD-11 audit (migration `1750000000145`)
Full ICD-11 audit of all 23 original UROL (Urology) diagnoses (see `MEDICAL_CODE_AUDITS/UROL/AUDIT_UROL.md`). UROL was **~74% corrupt (16/23 wrong + 1 invalid leaf)** — the seed mis-chaptered urological conditions onto **female-genital GA** codes (GA00=vulvitis, GA20, GA40), **cystitis GC00.x** (used for renal/bladder/ureter stones), and **nephritis GB4x/GB5x** blocks (GB40=nephritic, GB42, GB50=acute TIN, GB51=acute pyelonephritis), plus oncology mis-codes (RCC on ovary code 2C73; seminoma on vulva-melanoma code 2C70.1). 16 fixes in migration 145:
- **7 collision-aware MERGEs into existing shared rows** (UROL row deleted, shared row relinked): RCC `2C73.0`→`2C90.0` (SOC); seminoma `2C70.1`→`2C80.2` (PEDSURG/SOC); varicocele `GB40.0`→`BD75.1` (PEDSURG); torsion `GB42.0`→`GB01.0` (PEDSURG); VUR `GC10.0`→`GB56.5` (TRS); overactive bladder `GC80.0`→`GC50.0` (OBGYN); stress incontinence `GA00.0`→`MF50.20` (OBGYN).
- **8 free in-place recodes**: pyelonephritis `GA40.0`→`GB51`; kidney/bladder/ureter stones `GC00.0`/`GC00.2`/`GC00.1`→`GB70.0Z`/`GB71.0`/`GB70.1`; phimosis `GB50.0`→`GB05.2`; epididymo-orchitis `GB51.0`→`GB02.1`; hydronephrosis `GC20.0`→`GB56.4`; UPJO `GC21.0`→`GB56.0`; urethral stricture `GA20.0`→`GC03`.
- **1 invalid-leaf fix**: BPH `GA90.0`→`GA90` (Hyperplasia of prostate).
