...
/**
 * Production KA_CTS DB - for running INDEX-ONLY migrations (SQL_*_DEFAULT + SQL_DB_DEF_NAME_KA_CTS).
 * Only the four "Add*Index" migrations are registered. No table creation, no data changes.
 */
function getProductionIndexMigrationsKaCtsConfig(): DataSourceOptions {
  ...
  return {
    ...
    database: process.env.SQL_DB_DEF_NAME_KA_CTS!,
    synchronize: false,
...
export const ProductionIndexMigrationsKaCtsDataSource = new DataSource(
  getProductionIndexMigrationsKaCtsConfig()
);
