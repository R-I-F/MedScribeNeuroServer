...
import { updateAdditionalQuestionValidator } from "../validators/updateAdditionalQuestion.validator";
...
export class AdditionalQuestionsRouter {
  ...
  public async initRoutes() {
    ...
    // Update additional question flags (superAdmin, instituteAdmin only) – only boolean 0/1 fields
    this.router.put(
      "/:mainDiagDocId",
      extractJWT,
      institutionResolver,
      ...
      async (req: Request, res: Response) => {
        ...
        if (result.isEmpty()) {
          try {
            const resp = await this.additionalQuestionsController.handleUpdate(req, res);
            if (resp) {
              res.status(StatusCodes.OK).json(resp);
            } else {
              res.status(StatusCodes.NOT_FOUND).json({ error: "Additional question not found" });
