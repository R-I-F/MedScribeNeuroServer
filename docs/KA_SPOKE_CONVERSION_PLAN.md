...
> Status: ✅ IMPLEMENTED (Stages 0–E) on branch `migration/mysql-to-postgres` — 2026-07-11. tsc-clean and verified on staging `ka-institute` (hub = LibelusRefApi @ dataVersion 217.279); all changes **uncommitted** (guardrail 1: `main` never touched). Sole remaining behavioral gap: the **NS six-flag map is all-zero** — real KA values still need a read-only prod-tenant capture + title-keyed seed. Deliberately NOT done in Stage E (see that entry): retiring the gitignored/untracked legacy `scripts/` + the `mysql2` dep prune — those are irreversible deletions of untracked local tooling.
