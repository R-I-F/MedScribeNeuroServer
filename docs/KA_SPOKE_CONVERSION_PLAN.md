...
> **📈 SCOPE EXTENSION (2026-07-12): mirror widened to ALL 15 departments** (one tenant = Kasr Al Ainy, full institute) by OpenClaw; revised + verified by Claude — mirror structure faithful to the hub (departments / main_diags.departmentId / department_diagnoses / lecture_topics + lectures.topicId; users gained nullable departmentId), counts EXACT vs hub (196/1319/1529/1643/2075/141/3237; proc_cpts 1429 vs 1434 = 5 hub rows linked to no category, unreachable by definition). **Defect found+fixed in revision: list reads had no department filter** → added `referenceRead.provider.ts` + `?deptCode=` (default `REF_DEPT_CODE`=NS, case-insensitive, 404 unknown / 400 malformed) so the legacy NS frontend is byte-identical while other departments are selectable. Details in CLAUDE.md "Mirror WIDENED" section. NB: 5 category titles now duplicate across departments — future title-keyed seeds must scope by departmentId.
